...
ErrorCode add_char_to_active_message(Chat* chat, const char c) {
    ...
    if (chat->active_message.text_index < MAX_CHAT_LENGTH) { // TODO: Magic number!
        chat->active_message.text[chat->active_message.text_index++] = c;
    } else {
...
ErrorCode clear_active_message(Chat* chat) {
    ...
    chat->active_message.text_index = 0;
    return EC_OK;
}

bool has_active_text(Chat* chat) {
    return chat->active_message.text_index > 0;
}
