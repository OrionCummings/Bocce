// This file has been edited to remove the following GCC warnings:
    ...
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        case CLAY_RENDER_COMMAND_TYPE_TEXT: {
            ...
            if (strlen > temp_render_buffer_len) {
                ...
                if (temp_render_buffer) free(temp_render_buffer);
                temp_render_buffer = (char*)malloc((size_t)strlen);
                temp_render_buffer_len = strlen;
            }

            // Raylib uses standard C strings so isn't compatible with cheap slices, we need to clone the string to append null terminator
            memcpy(temp_render_buffer, textData->stringContents.chars, (size_t)textData->stringContents.length);
        ...
        case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
            ...
            DrawTexturePro(
                imageTexture,
                (Rectangle) {
                0, 0, (float)imageTexture.width, (float)imageTexture.height
            },
                (Rectangle){
        ...
        case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
            ...
            if (config->cornerRadius.topLeft > 0) {
                ...
                DrawRectangle((int)boundingBox.x, (int)boundingBox.y, (int)boundingBox.width, (int)boundingBox.height, CLAY_COLOR_TO_RAYLIB_COLOR(config->backgroundColor));
            }
            break;
        }
